...
#ifndef _FGEN_STRINGS_H_
#define _FGEN_STRINGS_H_
...
fgen_strtok(char *str, const char *delim, char *entries[], int maxtokens)
{
    int i = 0;
    char *saved;
 ...
 */
static __inline__ int
fgen_strqtok(char *str, const char *delim, char *argv[], int maxtokens)
{
 ...
 */
static __inline__ int
fgen_stropt(const char *list, char *str, const char *delim)
{
    ...
    if (buf) {
        ...
        int nb = fgen_strtok(buf, delim, argv, STR_MAX_ARGVS);
        if (nb < 0)
            return -1;
        for (int i = 0; i < nb; i++)
 ...
 */
static __inline__ char *
fgen_strdupf(char *str, char *newstr)
{
    if (str)
        free(str);
 ...
 */
static __inline__ int
fgen_strcnt(char *s, char c)
{
    return (s == NULL || *s == '\0') ? 0 : fgen_strcnt(s + 1, c) + (*s == c);
}
