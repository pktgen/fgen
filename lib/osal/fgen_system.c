...
#include <fgen_cycles.h>        // for fgen_rdtsc
#include <fgen_gettid.h>
#include <fgen_strings.h>        // for fgen_strtok
 ...
 */
unsigned
fgen_socket_id(unsigned lcore_id)
{
    if (lcore_id == FGEN_LCORE_INVALID)
        lcore_id = fgen_lcore_id();

    for (int socket = 0; socket < fgen_max_numa_nodes(); socket++) {
        char path[PATH_MAX];
...
unsigned
fgen_socket_id_self(void)
{
    return fgen_socket_id(FGEN_LCORE_INVALID);
}
...
parse_procfs_stat_value(int thread_idx, int idx, int *id)
{
    ...
    if ((f = fopen(filename, "r")) == NULL) {
        fgen_printf("%s(): cannot open sysfs value %s\n", __func__, filename);
        goto leave;
    }

    if (fgets(buf, sizeof(buf), f) == NULL) {
        fgen_printf("%s(): cannot read sysfs value %s\n", __func__, filename);
        goto leave;
    }

    n = fgen_strtok(buf, " ", toks, sizeof(toks) / sizeof(toks[0]));
    if (n < 0) {
        fgen_printf("%s(): cannot parse sysfs value %s\n", __func__, filename);
        goto leave;
    }
    if (idx > n) {
        fgen_printf("%s(): parse index out of range %d only %d\n", __func__, idx, n);
        goto leave;
    }

    if (idx == 1 || idx == 2) {
        fgen_printf("%s(): index %d is a string or char type\n", __func__, idx);
        goto leave;
    }
...
int
fgen_lcore_id_by_thread(int thread_idx)
{
    int lcore_id;
...
parse_sysfs_value(const char *filename, unsigned long *val)
{
    ...
    if ((f = fopen(filename, "r")) == NULL) {
        fgen_printf("%s(): Warning cannot open sysfs value %s\n", __func__, filename);
        return -1;
    }
...
unsigned
fgen_core_id(unsigned lcore_id)
{
    char path[PATH_MAX];
    unsigned long id;
...
err:
    fgen_printf("Error reading core id value from %s for lcore %u - assuming core 0\n", SYS_CPU_DIR,
               lcore_id);
    return 0;
}

uint16_t
fgen_device_socket_id(char *netdev)
{
    char path[PATH_MAX];
    unsigned long numa;
...
err:
    fgen_printf("Error reading numa id value from %s\n", netdev);
    return 0;
}

unsigned int
fgen_max_lcores(void)
{
    if (nb_lcores == 0) {
...
#define NS_PER_SEC 1E9
    ...
    if (clock_gettime(CLOCK_MONOTONIC_RAW, &t_start) == 0) {
        uint64_t ns, end, start = fgen_rdtsc();
